...
function _encode(
    ...
    LibJpeg.jpeg_create_compress(cinfo_ptr)
    ...
    LibJpeg.jpeg_set_defaults(cinfo_ptr)
    isnothing(colorspace) || LibJpeg.jpeg_set_colorspace(cinfo_ptr, jpeg_color_space(colorspace))
    isnothing(quality) || LibJpeg.jpeg_set_quality(cinfo_ptr, quality, true)
    ...
    LibJpeg.jpeg_mem_dest(cinfo_ptr, buf_ptr, bufsize)

    # compression stage
    LibJpeg.jpeg_start_compress(cinfo_ptr, true)
    ...
    while (cinfo.next_scanline < cinfo.image_height)
        ...
        LibJpeg.jpeg_write_scanlines(cinfo_ptr, row_pointer, 1);
    end
    LibJpeg.jpeg_finish_compress(cinfo_ptr)
    LibJpeg.jpeg_destroy_compress(cinfo_ptr)
