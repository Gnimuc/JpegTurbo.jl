...
function jpeg_decode(
    ...
    try
        ...
        LibJpeg.jpeg_create_decompress(cinfo_ptr)
        LibJpeg.jpeg_stdio_src(cinfo_ptr, infile)
        LibJpeg.jpeg_read_header(cinfo_ptr, true)
        ...
        LibJpeg.jpeg_calc_output_dimensions(cinfo_ptr)
...
function _jpeg_decode!(out::Matrix{<:Colorant}, cinfo::LibJpeg.jpeg_decompress_struct)
    ...
    LibJpeg.jpeg_start_decompress(cinfo_ptr)
    while cinfo.output_scanline < cinfo.output_height
        # TODO(johnnychen94): try if we can directly write to `out` without using `buf`
        GC.@preserve buf LibJpeg.jpeg_read_scanlines(cinfo_ptr, Ref(pointer(buf)), 1)
        copyto!(out_uint8, (cinfo.output_scanline-1) * row_stride + 1, buf, 1, row_stride)
    end
    LibJpeg.jpeg_finish_decompress(cinfo_ptr)
